...
export default function DashboardPage() {
  ...
  const [deliveryOrder, setDeliveryOrder] = useState<OrderListItem | null>(null);
  ...
  const parseOrders = useCallback((data: OrderListItem[] | { results?: OrderListItem[] }) => {
  ...
  const fetchOrders = useCallback(async (nextFilters: typeof initialFilters) => {
    ...
    return apiFetch<OrderListItem[] | { results?: OrderListItem[] }>(`/orders/${query ? `?${query}` : ''}`);
  }, []);
  ...
  const loadData = useCallback(async (activeFilters: typeof initialFilters) => {
    try {
      ...
      setOrders(parseOrders(orderData));
    } catch (error) {
  ...
  }, [fetchOrders, isAuthError, logout, parseOrders]);
  ...
  }, [fetchOrders, filters, isAuthError, logout, parseOrders]);

  const verifyOrder = useCallback(async (orderId: number) => {
    try {
      const updated = await apiFetch<OrderListItem>(`/orders/${orderId}/verify/`, {
        method: 'POST',
        body: JSON.stringify({}),
      });
      ...
      <div className="relative z-10 mx-auto grid w-full max-w-[1320px] gap-4 px-3 py-4 sm:px-4 md:gap-5 md:px-6 md:py-6 xl:px-8">
        <div className="flex flex-col items-start justify-between gap-3 lg:flex-row lg:items-center">
          ...
          <div className="grid w-full grid-cols-1 gap-2 sm:flex sm:w-auto sm:flex-wrap">
            ...
            <button
              className="inline-flex w-full items-center justify-center gap-2 rounded-xl border border-neutral-900 bg-neutral-900 px-3.5 py-3 text-sm font-semibold tracking-wide text-white transition hover:-translate-y-0.5 sm:w-auto dark:border-white dark:bg-white dark:text-neutral-950"
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <div className="grid gap-2 rounded-[18px] border border-neutral-300 bg-white/90 p-4 shadow-[0_0_0_1px_rgba(0,0,0,0.08),0_24px_58px_-36px_rgba(0,0,0,0.25)] backdrop-blur-sm md:p-5 dark:border-neutral-700 dark:bg-neutral-900/90 dark:shadow-[0_0_0_1px_rgba(255,255,255,0.08),0_24px_58px_-36px_rgba(255,255,255,0.12)]">
            <strong>Total</strong>
            <div className="text-3xl font-extrabold tracking-tight">{stats.total}</div>
          </div>
          <div className="grid gap-2 rounded-[18px] border border-neutral-300 bg-white/90 p-4 shadow-[0_0_0_1px_rgba(0,0,0,0.08),0_24px_58px_-36px_rgba(0,0,0,0.25)] backdrop-blur-sm md:p-5 dark:border-neutral-700 dark:bg-neutral-900/90 dark:shadow-[0_0_0_1px_rgba(255,255,255,0.08),0_24px_58px_-36px_rgba(255,255,255,0.12)]">
            <strong>Ordered</strong>
            <div className="text-3xl font-extrabold tracking-tight">{stats.ordered}</div>
          </div>
          <div className="grid gap-2 rounded-[18px] border border-neutral-300 bg-white/90 p-4 shadow-[0_0_0_1px_rgba(0,0,0,0.08),0_24px_58px_-36px_rgba(0,0,0,0.25)] backdrop-blur-sm md:p-5 dark:border-neutral-700 dark:bg-neutral-900/90 dark:shadow-[0_0_0_1px_rgba(255,255,255,0.08),0_24px_58px_-36px_rgba(255,255,255,0.12)]">
            <strong>Verified</strong>
            <div className="text-3xl font-extrabold tracking-tight">{stats.verified}</div>
          </div>
          <div className="grid gap-2 rounded-[18px] border border-neutral-300 bg-white/90 p-4 shadow-[0_0_0_1px_rgba(0,0,0,0.08),0_24px_58px_-36px_rgba(0,0,0,0.25)] backdrop-blur-sm md:p-5 dark:border-neutral-700 dark:bg-neutral-900/90 dark:shadow-[0_0_0_1px_rgba(255,255,255,0.08),0_24px_58px_-36px_rgba(255,255,255,0.12)]">
            <strong>Completed</strong>
            <div className="text-3xl font-extrabold tracking-tight">{stats.completed}</div>
          </div>
        </div>
        ...
        <div className="grid gap-3 rounded-[18px] border border-neutral-300 bg-white/90 p-4 shadow-[0_0_0_1px_rgba(0,0,0,0.08),0_24px_58px_-36px_rgba(0,0,0,0.25)] backdrop-blur-sm md:p-5 dark:border-neutral-700 dark:bg-neutral-900/90 dark:shadow-[0_0_0_1px_rgba(255,255,255,0.08),0_24px_58px_-36px_rgba(255,255,255,0.12)]">
          ...
          <div>
            ...
            <input
              ...
              placeholder="Search customer name or URL"
            />
          </div>
        </div>
                  ...
                  <thead>
                    <tr>
                      {['Customer', 'URL', 'Platform', 'Product', 'Package', 'Qty', 'Payment', 'Primary Ref', 'Previous Ref', 'New Ref', 'Status', 'Customer Status', 'Entry Time', 'Actions'].map((head) => (
                        <th
                          key={head}
                          className="border-b border-neutral-300 px-3 py-3 text-left text-[11px] uppercase tracking-[0.06em] text-neutral-500 dark:border-neutral-700 dark:text-neutral-300"
                        >
                          {head}
                        </th>
                      ))}
                  ...
                  <tbody>
                    {orders.map((order) => (
                      <tr key={order.id}>
                        ...
                        <td className="max-w-[210px] break-all border-b border-neutral-300 px-3 py-3 align-top dark:border-neutral-700">
                          <a href={order.url} target="_blank" rel="noreferrer">{order.url}</a>
                        </td>
                        ...
                        <td className="border-b border-neutral-300 px-3 py-3 align-top dark:border-neutral-700">
                          ...
                          <div className="text-xs text-neutral-500 dark:text-neutral-300">{order.payment_medium}</div>
                        </td>
                        ...
                        <td className="border-b border-neutral-300 px-3 py-3 align-top dark:border-neutral-700">
                          <span className={`inline-flex rounded-full border border-current px-2.5 py-1 text-[11px] font-bold uppercase tracking-wider ${statusClass[order.status]}`}>
                            {order.status}
                          </span>
                        </td>
                        ...
                        <td className="border-b border-neutral-300 px-3 py-3 align-top dark:border-neutral-700">
                          {order.status === 'ordered' && (
                            <button
                              className="inline-flex items-center justify-center gap-2 rounded-lg border border-neutral-300 bg-white/40 px-3 py-2 text-xs font-medium transition hover:-translate-y-0.5 dark:border-neutral-700 dark:bg-neutral-900/40"
                          ...
                          {order.status === 'completed' && (
                            <span className="text-sm text-neutral-500 dark:text-neutral-300">Done</span>
                          )}
                        </td>
                      </tr>
                    ))}
                    {orders.length === 0 && (
                      <tr>
                        <td colSpan={14} className="border-b border-neutral-300 px-3 py-4 text-sm text-neutral-500 dark:border-neutral-700 dark:text-neutral-300">
                          No orders found.
                        </td>
                      </tr>
                    )}
                  ...
                  <div key={order.id} className="rounded-2xl border border-neutral-300 bg-white/80 p-3 dark:border-neutral-700 dark:bg-neutral-900/80">
                    <div className="mb-2 flex items-start justify-between gap-2">
                      <div>
                        ...
                        <a href={order.url} target="_blank" rel="noreferrer" className="text-xs break-all text-neutral-500 dark:text-neutral-300">
                          {order.url}
                        </a>
                      </div>
                      <span className={`inline-flex rounded-full border border-current px-2 py-1 text-[10px] font-bold uppercase tracking-wider ${statusClass[order.status]}`}>
                        {order.status}
                      </span>
                    </div>
